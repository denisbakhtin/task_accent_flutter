...
class ServiceController<T> implements EventEntitySink<T> {
  ...
  Future<T> get({int id, @required String url}) async {
    ...
    switch (response.statusCode) {
      case 200:
        {
          _element = _fromJson(response.body);
      ...
      case 200:
        {
          _element = _fromJson(response.body);
          add(_element);
          //big question here, if comparison would work. Need to implement == operator? Or call getList in component's listen method
  ...
  Map<String, dynamic> _toJson(T element) {
    if (T == User) return (element as User).toJson();
    if (T == Project) return (element as Project).toJson();
    if (T == Task) return (element as Task).toJson();
    if (T == Category) return (element as Category).toJson();
    if (T == Session) return (element as Session).toJson();
    if (T == TaskLog) return (element as TaskLog).toJson();
    if (T == Comment) return (element as Comment).toJson();
    //add other classes accordingly, otherwise meet this error :P

    throw UnsupportedError("toJson on $T");
  }
}
